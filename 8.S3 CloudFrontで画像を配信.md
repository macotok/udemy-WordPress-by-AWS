...
- Default Cache Behavior Settingsはデフォルト値のまま
...
|  画面  |  押下  |
| ---- | ---- |
|  サービス/CloudFront/CloudFront Distributions  |  作成したCloudFrontのID/Edit

- `Alternate Domain Names`にドメインを指定。例 static.hoge.com
- CloudFrontで独自ドメインを設定する場合はSSL証明書が必要なので、SSL証明書を発行
    - `Request or Import a Certificate with ACM`を押下
    - ドメイン名の追加でサブドメインを*に指定。例 *.hoge.com
    - `この証明書に別の名前を追加`でドメインを指定。例 hoge.com
    - `次へ`を押下
    - ` DNS の検証`をチェック
    - `タグを追加`は設定なしで`確認とリクエスト`を押下
    - `検証`画面で`Route 53 でのレコードの作成`を押下してそのまま`作成`を押下
    - `続行`を押下
- `SSL Certificate`で`Custom SSL Certificate`を選択してSSL証明書発行済みのサブドメインが*のドメインを選択
- 残りの入力はデフォルト値のまま、`Yes,Edit`を押下
...
|  画面  |  押下  |
| ---- | ---- |
|  サービス/Route53/ホストゾーン/ドメイン | レコードセットの作成
...
- WordPressの設定画面で`Custom Domain (CNAME)`をONにする
- 独自ドメインを入力
